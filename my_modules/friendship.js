...
module.exports = {
    ...
    OnMessage(message) {
        ...
        if (
            message.content.startsWith(
                getCurrentServer(message.guild.id).config.prefix
            )
        )
            return;
        ...
        userController.updateFriendship(
            message.author.id,
    ...
    OnVoiceStateUpdate(oldState, newState) {
        ...
        if (oldState.channel === null && newState.channel !== null) {
            ...
            userController.readUser(newState.member.user.id).then((doc) => {
                if (!doc) return;
                if (
                    doc.friendship > THRESHOLDS[ON_CONNECTION_GREET_INDEX_TH] &&
                        ...
                        user.send(
                            doc.username +
                                strings.getRandomString(
                                    "HELLO",
                                    newState.guild.guild_id
                                )
                        )
                            .then((msg) => {
                                // Delete message after timeout.
                                msg.delete({ timeout: 60 * 1000 }).catch(
                                    (err) => {
                                        console.log(
                                            "No se ha podido borrar el mensaje."
                                        );
                                    }
                                );
                            })
    ...
    GetFriendship(message) {
        userController
            .readUser(message.author.id, message.guild.id)
            .then((user) => {
                if (user) {
                    for (let i = 0; i < THRESHOLDS.length - 1; i++) {
                        if (
                            user.friendship > THRESHOLDS[i] &&
                            user.friendship <= THRESHOLDS[i + 1]
                        ) {
                            message.channel.send(
                                strings.getModuleString(
                                    "FRIENDSHIP",
                                    "MESSAGE_HEADER",
                                    message.guild.id
                                ) +
                    ...
                    userController
                        ...
                        .then((newUser) => {
                            for (let i = 0; i < THRESHOLDS.length - 1; i++) {
                                if (
                                    newUser.friendship > THRESHOLDS[i] &&
                                    newUser.friendship <= THRESHOLDS[i + 1]
                                ) {
                                    message.channel.send(
                                        strings.getModuleString(
                                            "FRIENDSHIP",
