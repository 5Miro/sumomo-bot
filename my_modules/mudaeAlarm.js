...
module.exports = {
	name: "mudaeAlarm",
	isActivated: true,
	descrip: ["Sumomo sends DMs as reminders of Mudae's resets", "Sumomo envía MPs recordando sobre los resets de Mudae"],
	OnInterval() {
		...
		guilds.forEach((guild) => {
			if (date.getUTCMinutes() == guild.config.mudae.initial_minutes && date.getUTCSeconds() == 0) {
				// First check MINUTES
				// Rolls reset.
				// Now check HOURS
...
async function ringAlarm(string, guild_id) {
	...
	for (const doc of users) {
		...
		client.users.fetch(doc.user_id).then((user) => {
			user.send(doc.username + string)
				.then((msg) => {
					// Delete message after timeout.
					msg.delete({ timeout: globals.rolls_interval * 60 * 60 * 1000 }).catch((err) => {
						// timeout is equal to rolls time interval in miliseconds.
