...
const LANG_EN = {
    // MUDAE ALARM
    RESET_ROLLS: ["-sama! rolls, rolls, rolls have arrived! - *whistles* - ", "-sama, get up, get up!, it's time to roll! (^‿^)", "-sama, we have to roll, don't miss it! - *tambourine noises* -"],
    RESET_CLAIMS: ["-sama, heads up! time to claim, let's go! (ง '̀͜ '́ )ง ", "-sama! you can claim, you can claim! - *tambourine noises* - ", "-sama, let's claim!, let's claim! - *whistles* -"],
    ...
    HELLO: ["-sama! okaeri! okaeri! ٩(•ᗜ•)٩ - *tambourine noises* -", "-sama, why don't we work out together today? (ง '̀͜ '́ )ง", "-sama is back! let's dance and sing! ᕙ(`▽´)ᕗ", "-sama, did you know I'm 16cms height? (•ᗜ•)ﾉ", "-sama! to start a good day we gotta move our body! (•ᗜ•)ﾉ"],

    FRIENDSHIP: ["It seems like it doesn't like you.", "She's not used to you quite yet.", "She's beginning to gain confidence in your presence.", "She likes you. She looks so happy!", "She fully trusts you. You two are inseparable!", "She adores you! There's nothing more to say, how wonderful."],
    ...
    COMMANDS: {
        ...
        SETPREFIX: {
            ...
            ERROR: "Syntax error.\nPrefix must contain a minimum of " + globals.PREFIX_MIN_LENGTH + "characters and a maximum of " + globals.PREFIX_MAX_LENGTH + " (•ᗜ•)ﾉ",
        }
    },
    MODULES: {
        FRIENDSHIP: {
...
const LANG_ES = {
    // MUDAE ALARM
    RESET_ROLLS: ["-sama! rolls, rolls, llegaron los rolls! - *silbatazos* - ", "-sama, arriba, arriba!, es hora de rollear! (^‿^)", "-sama, es momento de rollear, no se lo pierda! - *ruidos de pandereta* -"],
    RESET_CLAIMS: ["-sama, atención! es hora de claimear, vamos! (ง '̀͜ '́ )ง ", "-sama! ya tiene claim, ya tiene claim! - *ruidos de pandereta* - ", "-sama, a claimear!, a claimear! - *silbatazos* -"],
    ...
    COMMANDS: {
        ...
        SETPREFIX: {
            ...
            ERROR: "Error de sintáxis.\nEl prefijo debe tener un mínimo de" + globals.PREFIX_MIN_LENGTH + "caracteres y un máximo de " + globals.PREFIX_MAX_LENGTH + " (•ᗜ•)ﾉ",
        }
    },
    MODULES: {
        FRIENDSHIP: {
...
module.exports = {
    getRandomString(string, guild_id) {
         ...
         */
        return STRINGS_LIBRARY[getServerLanguageIndex(guild_id)][string][Math.floor(Math.random() * STRINGS_LIBRARY[getServerLanguageIndex(guild_id)][string].length)];
    },
...
// GIFS
const GIFS = {
    DANCE: ["https://tenor.com/view/sumomo-chobits-anime-wakey-wakey-plum-gif-15044233", "https://j.gifs.com/q7Njn0.gif", "https://j.gifs.com/xnXqZr.gif", "https://j.gifs.com/OM4lOp.gif", "https://j.gifs.com/nxKZj7.gif", "https://j.gifs.com/WL4nRx.gif"],
